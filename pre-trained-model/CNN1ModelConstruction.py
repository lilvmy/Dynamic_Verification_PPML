...
def setup_logging():
    ...
    handler = log.FileHandler("log/cnn1_model_for_mnist.log", "w", "utf-8")
...
class CNN1(nn.Module):
    """
    CNN1 model implementation.
    """

    def __init__(self, num_classes):
        """
        Initialize the CNN1 model.
        """
    ...
    def forwardRegular(self, x):
        """
        Forward pass of the regular cnn1 model.
        """
        out = self.cnn1(x)
        return out
...
def testing(test_loader, device, model):
    """
    Test the CNN1 model with inputs.
    """
    correct = 0
    total = 0
    with torch.no_grad():
...
def main():
    """
    Main function to set up, train, and experiments the CNN1 model in both original and fixed-point representation settings.
    """
