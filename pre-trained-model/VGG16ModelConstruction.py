...
def setup_logging():
    ...
    handler = log.FileHandler("./log/VGG16_model_for_CIFAR10.log", "w", "utf-8")
...
def testing(test_loader, device, model):
    """
    Test the VGG16 model with inputs.
    """
    correct = 0
    total = 0
    with torch.no_grad():
...
def main():
    ...
    print("\nSave Model...")
    torch.save(model.state_dict(), "./model/VGG16_model.pt")

if __name__ == "__main__":
    # setup_logging()
    # main()
    model = torch.load("/home/lilvmy/paper-demo/Results_Verification_PPML/pre-trained-model/model/VGG16_model.pt", weights_only=True)
    model_params = {}
    for name, param in model.items():
        model_params[name] = param.cpu().numpy()

    np.save("./model/VGG16_model_params.npy", model_params)

    load_params = np.load("./model/VGG16_model_params.npy", allow_pickle=True).item()
