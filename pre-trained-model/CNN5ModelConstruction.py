...
def setup_logging():
    ...
    handler = log.FileHandler("log/cnn5_model_for_mnist.log", "w", "utf-8")
...
class CNN5(nn.Module):
    """
    CNN5 model implementation.
    """

    def __init__(self, num_classes):
        """
        Initialize the CNN5 model.
        """
    ...
    def forwardRegular(self, x):
        """
        Forward pass of the regular cnn5 model.
        """
        out = self.cnn5(x)
        return out
...
def testing(test_loader, device, model):
    """
    Test the CNN5 model with inputs.
    """
    correct = 0
    total = 0
    with torch.no_grad():
...
def main():
    ...
    print("\nSave Model...")
    torch.save(model.state_dict(), "./model/cnn5_model.pt")

if __name__ == "__main__":
    # setup_logging()
    # main()

    model = torch.load("/home/lilvmy/paper-demo/Results_Verification_PPML/pre-trained-model/model/cnn5_model.pt", weights_only=True)
    model_params = {}
    for name, param in model.items():
        model_params[name] = param.cpu().numpy()

    np.save("./model/cnn5_model_params.npy", model_params)

    load_params = np.load("./model/cnn5_model_params.npy", allow_pickle=True).item()
