...
def main():
    """
    Main function to set up, train, and experiments the CNN2 model in both original and fixed-point representation settings.
    """
...
if __name__ == "__main__":
    ...
    model = torch.load("/home/lilvmy/paper-demo/Results_Verification_PPML/pre-trained-model/model/cnn2_model.pt", weights_only=True)
    model_params = {}
    for name, param in model.items():
        model_params[name] = param.cpu().numpy()

    np.save("./model/cnn2_model_params.npy", model_params)

    load_params = np.load("./model/cnn2_model_params.npy", allow_pickle=True).item()
