...
def setup_logging():
    ...
    handler = log.FileHandler("log/cnn4_model_for_mnist.log", "w", "utf-8")
...
class CNN4(nn.Module):
    """
    CNN4 model implementation.
    """

    def __init__(self, num_classes):
        """
        Initialize the CNN4 model.
        """
        super(CNN4, self).__init__()
        self.cnn4 = nn.Sequential(
            Conv2d(1, 256, kernel_size=3),
            Square(),
    ...
    def forwardRegular(self, x):
        """
        Forward pass of the regular cnn4 model.
        """
        out = self.cnn4(x)
        return out
...
def testing(test_loader, device, model):
    """
    Test the CNN4 model with inputs.
    """
    correct = 0
    total = 0
    with torch.no_grad():
...
def main():
    """
    Main function to set up, train, and experiments the CNN4 model in both original and fixed-point representation settings.
    """
    ...
    print("\nSave Model...")
    torch.save(model.state_dict(), "./model/cnn4_model.pt")

if __name__ == "__main__":
    # setup_logging()
    # main()

    model = torch.load("/home/lilvmy/paper-demo/Results_Verification_PPML/pre-trained-model/model/cnn4_model.pt", weights_only=True)
    model_params = {}
    for name, param in model.items():
        model_params[name] = param.cpu().numpy()

    np.save("./model/cnn4_model_params.npy", model_params)

    load_params = np.load("./model/cnn4_model_params.npy", allow_pickle=True).item()
