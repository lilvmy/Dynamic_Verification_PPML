...
def setup_logging():
    ...
    handler = log.FileHandler("log/cnn3_model_for_mnist.log", "w", "utf-8")
...
class CNN3(nn.Module):
    ...
    def forwardRegular(self, x):
        """
        Forward pass of the regular cnn3 model.
        """
        out = self.cnn3(x)
        return out
...
def testing(test_loader, device, model):
    """
    Test the CNN3 model with inputs.
    """
    correct = 0
    total = 0
    with torch.no_grad():
...
def main():
    """
    Main function to set up, train, and experiments the CNN3 model in both original and fixed-point representation settings.
    """
    ...
    print("\nSave Model...")
    torch.save(model.state_dict(), "./model/cnn3_model.pt")

if __name__ == "__main__":
    # setup_logging()
    # main()

    model = torch.load("/home/lilvmy/paper-demo/Results_Verification_PPML/pre-trained-model/model/cnn3_model.pt", weights_only=True)
    model_params = {}
    for name, param in model.items():
        model_params[name] = param.cpu().numpy()

    np.save("./model/cnn3_model_params.npy", model_params)

    load_params = np.load("./model/cnn3_model_params.npy", allow_pickle=True).item()
